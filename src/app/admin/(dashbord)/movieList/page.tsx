...
export default function MovieList() {
  ...
  const handleUpdateStatus = async (id: string, status: "available" | "coming_soon") => {
    try {
                ...
                {movies.map((movie) => (
                  <tr key={movie._id} className="group hover:bg-[#1a1c40] transition-colors duration-200">
                    <td className="px-6 py-4">
                      <div className="relative w-12 h-16 rounded-lg overflow-hidden border border-gray-700 shadow-lg">
                        <Image
